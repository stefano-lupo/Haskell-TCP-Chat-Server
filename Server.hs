...
data Server = Server
    ...
    , serverClientsByName   :: TVar (Map ClientName Client),
      serverChatRooms       :: TVar (Map ChatRoomId ChatRoom)
    }
...
serve server@Server{..} id handle = do
    ...
    if null name
        then hPutStrLn handle "Bye, anonymous coward"
            ...
            bracket_ (atomically $ handleClientJoin server client)
...
handleClientJoin server@Server{..}
    ...
    case Map.lookup 99 chatRoomMap of
        Nothing -> do
            chatRoom <- initChatRoom 99 "My First ChatRoom"
            broadcast server $ Notice $ chatRoomName chatRoom
        Just chatroom -> do
            broadcast server $ Notice $
               "Chatroom found"
